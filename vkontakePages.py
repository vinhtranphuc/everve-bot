...
idBlackList = []
isStop = False
targetUrl = 'https://everve.net/tasks/vkontakte-pages/'
...
def newSubscribe(event, browser, idStr):
...
def subscribeVkontakte():
    ...
    for id in availbleList:
        thread = threading.Thread(target=newSubscribe, args=(event, initTab.browser, id),daemon=True)
        thread.start()
        # thread.join()
    ...
    if (isFinish):
        ...
        for idx, tab in enumerate(initTab.browser.tabs):
            if "everve.net/tasks/vkontakte-pages" in tab.url:
