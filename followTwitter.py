...
idBlackList = []
isStop = False
targetUrl = 'https://everve.net/tasks/twitter-followers/'

def doFollow(twitterTab):
    print('Start Do follow :', twitterTab.url)
    try:
        ...
        print('+Error do follow twitter')
        print(e.__cause__)
        # newTab.close()
    finally:
        print('End Do follow :', twitterTab.url)
        twitterTab.close()

def newFollowTwitter(event, browser, idStr):
...
def followTwitter():
    ...
    for id in availbleList:
        thread = threading.Thread(target=newFollowTwitter, args=(event, initTab.browser, id),daemon=True)
        thread.start()
        # thread.join()
    ...
    if (isFinish):
        for idx, tab in enumerate(initTab.browser.tabs):
            if(idx == 0):
                continue
            if not "everve.net/tasks/twitter-followers" in tab.url:
                doFollow(tab)
        
        for idx, tab in enumerate(initTab.browser.tabs):
            if "everve.net/tasks/twitter-followers" in tab.url:
